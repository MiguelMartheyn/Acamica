...
class Politica extends React.Component {
  componentDidMount() {
    this.props.fetchCategoria(1);
  }

  render() {
    const { noticiasCategoria } = this.props;

    let politica = noticiasCategoria.filter(
      (filter) => filter.category === "Política" && filter.img_url !== null
    );
    const politicaList = politica.slice(0, 10);
    return (
      <div className="news">
        <h1>Política</h1>
        {politicaList ? (
          <ul>
            {politicaList.map((quotes) => (
              <a
                key={quotes.news_id}
                href={quotes.url}

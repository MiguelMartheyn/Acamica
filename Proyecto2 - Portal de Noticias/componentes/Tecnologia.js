...
class Tecnologia extends React.Component {
  componentDidMount() {
    this.props.fetchCategoria(3);
  }
  render() {
    const { noticiasCategoria } = this.props;

    let tecnologia = noticiasCategoria.filter(
      (filter) => filter.category === "Tecnología" && filter.img_url !== null
    );
    const tecnologiaList = tecnologia.slice(0, 10);
    return (
      <div className="news">
        <h1>Tecnología</h1>
        {tecnologiaList ? (
          <ul>
            {tecnologiaList.map((quotes) => (
              <a
                key={quotes.news_id}
                href={quotes.url}
